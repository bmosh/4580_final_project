    ...
    def visualize_final_level_gender_dists(self):
        for level in range(1,5):
            ...
            plt.plot([i for i in range(-1, self.num_rounds+2)], [np.mean(self.male_round_gender_dists[level-1]) \
                                                                 for i in range(-1, self.num_rounds+2)], color="purple", label="mean proportion of men in level")
            plt.plot([i for i in range(-1, self.num_rounds+1)], self.get_steady_state_gender_dist()[level] * np.ones(self.num_rounds+2), color="orange", label="steady state mean proportion of men")
